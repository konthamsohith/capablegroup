...
const TechStackBuilder: React.FC = () => {
    ...
    const addedByDropRef = useRef<string | null>(null);
    ...
    const getPointerPosition = (e: PointerEvent | MouseEvent | TouchEvent, info?: { point?: { x: number; y: number } }): { x: number; y: number } => {
        if (info?.point != null && typeof info.point.x === 'number' && typeof info.point.y === 'number') {
            return { x: info.point.x, y: info.point.y };
        }
        const ev = e as MouseEvent & { changedTouches?: TouchList };
    ...
    const isPointerInDropZone = (clientX: number, clientY: number, padding = 0): boolean => {
    ...
    const DROP_ZONE_PADDING = 24;

    const onDragEnd = (e: PointerEvent | MouseEvent | TouchEvent, info: { point?: { x: number; y: number } }, techId: string) => {
        setIsDraggingOver(false);
        const { x: clientX, y: clientY } = getPointerPosition(e, info);
        if (isPointerInDropZone(clientX, clientY, DROP_ZONE_PADDING)) {
            if (!selectedIds.includes(techId)) {
                addedByDropRef.current = techId;
                toggleTech(techId);
            }
        }
    };

    const onDrag = (_e: PointerEvent | MouseEvent | TouchEvent, info: { point?: { x: number; y: number } }) => {
        const { x, y } = getPointerPosition(_e, info);
        setIsDraggingOver(isPointerInDropZone(x, y, DROP_ZONE_PADDING));
    };

    const selectedTech = allTech.filter(t => selectedIds.includes(t.id));
            ...
            <div className="container mx-auto px-4 sm:px-6 relative z-10 w-full max-w-7xl">
                ...
                <div className="text-center mb-16 md:mb-24">
                    <motion.div
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-black/5 text-[#000000] text-xs font-bold tracking-widest uppercase mb-8"
                    >
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 relative min-h-[400px] lg:min-h-[600px] w-full items-start">
                                ...
                                <div className="flex flex-col gap-3 w-full">
                                    {allTech.filter(t => t.category === cat).map(tech => {
                                        ...
                                        <motion.button
                                            ...
                                            dragMomentum={false}
                                            onDrag={onDrag}
                                            onDragEnd={(e, info) => onDragEnd(e, info, tech.id)}
                                            whileHover={!isSelected ? { scale: 1.02, y: -2 } : {}}
                                            whileTap={!isSelected ? { scale: 0.95 } : {}}
                                            whileDrag={{ scale: 1.05, zIndex: 9999, cursor: 'grabbing' }}
                                            onClick={() => {
                                                if (addedByDropRef.current === tech.id) {
                                                    addedByDropRef.current = null;
                                                    return;
                                                }
                                                toggleTech(tech.id);
                                            }}
                                            className={`p-3 lg:p-4 flex flex-row items-center gap-3 rounded-[16px] font-bold text-[13px] transition-all duration-300 shadow-sm touch-none text-left w-full relative overflow-hidden group ${
                                                isSelected
                                                    ? 'bg-gray-100 text-gray-500 border border-gray-300 shadow-none cursor-default opacity-75'
                                        ...
                                        >
                                            ...
                                            <div className={`w-5 h-5 flex-shrink-0 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                                                isSelected
                    ...
                    <div className="w-full lg:w-1/3 flex-shrink-0 sticky top-32 self-start z-30">
                        <motion.div 
                            ref={dropZoneRef}
                            animate={{ 
                                borderColor: isDraggingOver ? '#ff6321' : '#e5e7eb',
                                backgroundColor: isDraggingOver ? '#f9fafb' : '#ffffff',
                                scale: isDraggingOver ? 1.01 : 1
                            }}
                            className={`rounded-[32px] p-8 md:p-10 border border-gray-200 bg-white text-gray-900 relative overflow-hidden transition-colors duration-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]`}
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="min-h-[260px] max-h-[320px] overflow-y-auto mb-2 pr-2 custom-scrollbar">
                                    {selectedIds.length === 0 ? (
                                        ...
                                        <div className="flex flex-col gap-2 pb-28 pt-2">
                                            <AnimatePresence mode="popLayout">
                                                {selectedTech.map(tech => (
                                                    <motion.div
                                                        ...
                                                        className="w-full flex items-center justify-between p-4 bg-white text-gray-900 font-medium text-[13px] rounded-xl border border-gray-200 shadow-sm group hover:border-[#ff6321] transition-colors"
                                                    >
                                                        <div className="flex items-center gap-4">
                                                            <div className="w-8 h-8 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-700 p-1.5 flex-shrink-0">
                                                                {tech.icon}
                                                            </div>
                                                            <div className="flex flex-col text-left">
                                                                <span className="leading-tight text-[14px] font-bold">
                                                                    {tech.name}
                                                                </span>
                                ...
                                <div className="relative z-30 pt-4 border-t border-gray-100">
                                    <motion.button
                                        ...
                                        className={`w-full py-4 rounded-[16px] font-bold text-[13px] tracking-widest uppercase transition-all duration-300 ${
                                            selectedIds.length > 0 
                        ...
                        >
                            <div className="p-6">
                                ...
                                <p className="text-[13px] text-[#69686e] leading-relaxed mb-4">
                                    We've captured your {selectedIds.length}-tool configuration. Standard engagement: <span className="font-medium text-black">Enterprise</span>, estimated delivery <span className="font-medium text-black">6–8 weeks</span>.
                                </p>
                                ...
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        className="flex-1 py-3 px-4 bg-black text-white rounded-xl text-[13px] font-semibold hover:bg-[#1a1a1a] transition-colors"
                                    >
                                        Request quote
                                    </button>
                                    <button
